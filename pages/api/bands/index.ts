...
handler.post(async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  // in this app.
  // This endpoint will be hit by testing directly to test on-demand ISR revalidation
  ...
  // add a band (here is where authorization would be validated)
  const { newBand } = req.body;
  const addedBand = await addBand(newBand);

  // revalidate bands page for ISR
  // notes: this will change to `res.revalidate` when
